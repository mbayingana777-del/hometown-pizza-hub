...
import { Plus, Check, Flame, Settings2 } from "lucide-react";
...
export function MenuItemCard({ item }: { item: MenuItem }) {
  ...
  return (
    <>
      <div className="group relative flex flex-col overflow-hidden rounded-3xl border border-border/60 bg-card shadow-card transition-all hover:shadow-deep hover:-translate-y-0.5">
        ...
        <div className="flex flex-1 flex-col p-4">
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            <button
              onClick={handleAdd}
              className={`flex h-11 items-center gap-1.5 rounded-full px-4 text-sm font-bold transition-all active:scale-95 ${
                added
                  ? "bg-italian text-accent-foreground"
      ...
      <ToppingsModal item={item} open={modalOpen} onOpenChange={setModalOpen} />
    </>
  );
}
